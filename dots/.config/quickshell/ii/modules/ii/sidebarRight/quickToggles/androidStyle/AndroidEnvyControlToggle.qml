...
AndroidQuickToggleButton {
    ...
    // Start hidden by default - only show if envycontrol is available
    visible: false
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode === 0) {
                // envycontrol exists, now verify daemon communication
    ...
    // Validate that envycontrol can communicate with daemon
    Process {
        id: validateDaemon
