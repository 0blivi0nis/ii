...
Singleton {
    ...
    function syncCalendars() {
        root.isLoading = true
        // console.log("[CalendarService] syncCalendars invoked; useVdirsyncer=", Config.options.calendar.useVdirsyncer)
        if (Config.options.calendar.useVdirsyncer) {
    ...
    Timer {
        id: interval
        running: false
        interval: Config.options.calendar.syncInterval * 60000
        repeat: true
        onTriggered: {
            getEventsProcess.running = true
        }
    ...
    Timer {
        id: syncInterval
        running: Config.options.calendar.useVdirsyncer
